...
`connectd` never blindly reads as fast as a peer can send. Its read side and write side are deliberately coupled so that it never queues more than it can deliver: the read loop reads **one** message and then parks itself on a wake token, `&peer->peer_in`, instead of immediately reading the next one.
...
It stays asleep until the write side wakes that token. And the write side only wakes it once the outgoing buffer has actually drained onto the socket:

```c
if (have_empty_encrypted_queue(peer)) {
        io_wake(&peer->subds);
        io_wake(&peer->peer_in);   /* only now is it OK to read the next message */
...
`next_read` reads again right away. It never parks on `&peer->peer_in`, so the backpressure coupling that protects every other message path simply does not apply to pings and gossip queries.

The two messages that take this shortcut are the ones where the peer picks the reply size:
...
The outgoing queue grows without bound. `query_channel_range` with `number_of_blocks` set to `U32::MAX` does the same thing, piling enormous gossip replies onto the same queue.
...
The fix routes locally-handled messages back through the same read gate as everything else. Instead of reading the next message immediately, `connectd` now nudges the write side and then parks on `&peer->peer_in`:
...
Now a `ping` or gossip query takes the identical path as a subdaemon-bound message. After queuing the reply, the read loop sleeps on `&peer->peer_in` and does not read again until the write side has flushed the previous reply and woken it. Against a peer that never reads, the writer never flushes, so reading stalls after a single queued reply. `peer_outq` can no longer grow past roughly one message, and the flood can no longer drive the node to OOM.

It landed as [PR #8525](https://github.com/ElementsProject/lightning/pull/8525).

A complementary change landed shortly after, throttling all incoming peer traffic to 1 MB per second, which caps how fast an attacker can drive request processing in the first place.
